...
// Near Protocol isValidAccount
function isValidAccountNEAR(accountId: string): boolean {
...
function nearAddrEncoder(data: Buffer): string {
    // Code converted from:
...
function nearAddrDecoder(data: string): Buffer {
    // check isValidAccountID
    const validID = isValidAccountNEAR(data);

    if (!validID) {
        throw Error('Unrecognised address Account format');
    }

    const buffer = decodeUnsafe(data);
...
export const formats: IFormat[] = [
  ...
  getConfig('NEAR', 397, nearAddrEncoder, nearAddrDecoder),
